<template>
  <div class="mt-8 border-t border-purple-100 pt-6 no-print">
    <div class="flex justify-between items-center mb-5">
      <div class="flex items-center space-x-2">
        ...
        <div>
          ...
          <p class="text-[10px] text-gray-400">활동 기록을 분석하여 자율활동 특기사항 문구를 제안합니다.</p>
        </div>
      </div>
      <button 
        @click="handleGenerate"
        :disabled="isGenerating"
        class="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl text-xs font-bold hover:shadow-lg disabled:from-gray-300 disabled:to-gray-400 transition-all flex items-center"
      >
        <span v-if="isGenerating" class="inline-block animate-spin mr-2">↻</span>
        {{ isGenerating ? 'AI 분석 중...' : '🤖 자율활동 초안 생성' }}
      </button>
    </div>

    <div v-if="result" class="animate-fade-in">
      <div class="bg-white p-6 rounded-2xl border-2 border-purple-100 hover:border-purple-300 transition-all shadow-sm group">
        <div class="flex justify-between items-center mb-3">
          <div class="flex items-center space-x-2">
            <span class="text-[10px] font-black text-purple-600 uppercase tracking-widest">자율활동 특기사항</span>
    ...
    <div v-else-if="!isGenerating" class="text-center py-12 bg-purple-50/30 rounded-3xl border-2 border-dashed border-purple-100">
      <p class="text-xs text-purple-400 font-bold">학생의 활동 기록을 토대로 자율활동 문구가 생성됩니다.</p>
    </div>
  </div>
</template>
...
const handleGenerate = async () => {
  try {
    ...
    if (settingsDoc.exists()) {
      dbApiKey = settingsDoc.data().geminiApiKey;
      customPrompt = settingsDoc.data().recordPrompt || customPrompt;
    }

    const activityText = props.subTab === 'logs' 
      ? `[독서 기록] 도서명: ${props.student.title}, 내용: ${props.student.summary}, 깨달은 점: ${props.student.realization}`
    ...
    const data = await aiService.generateStructuredRecord(dbApiKey, prompt, recordSchema);
...
const copyText = (text) => {
  ...
  alert("클립보드에 복사되었습니다. 나이스(NEIS) 자율활동란에 붙여넣으세요.");
};
